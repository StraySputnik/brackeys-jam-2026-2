...
static void update_entity(Entity* entity, Game* game, const float delta_time) {
    ...
    case PROJECTILE:
        ...
        if (entity->position.x < -1000 || entity->position.x > 5000 || entity->position.y < -1000 || entity->position.y > 5000) {
            despawn_entity(&game->scene.entity_pool, entity->id);
            break;
        }
...
Game load_game(const char* filename, const DrawConfig draw_config) {
    ...
    game.camera = (Camera2D){
        ...
        .rotation = 0.0f,
        .target = (Vector2){.x = (float)grid_width - 640.0f, .y = (float)grid_height - 360.0f},
        .zoom = 1.0f,
    };
...
void handle_ui(Game* game) {
    ...
    if (building == TOWER) {
        DrawCircle(
            ...
            (Color){.r = placeholder_color.r, .g = placeholder_color.g, .b = placeholder_color.b, .a = 100}
        );
    }
